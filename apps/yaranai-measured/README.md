...
- **誓いのなか・卒業済みのアプリは「時間の行き先」の足切りを免除。** 候補一覧の
  ノイズ足切り(週合計5分未満・12週平均0.5分未満・上限30件)は誓いの無いアプリ
  だけに掛け、誓いのあるアプリは1分でも使えば並び続ける。「一覧から消えた =
  卒業できる」の対応はこの免除で保証する(候補窓は当日を含む7日で、卒業判定の
  窓とは1日ずれるため)。
