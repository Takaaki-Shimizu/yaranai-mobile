...
// 単調非減少ガード(非交渉ライン4)。一度見せた蓄積より庭が後退しないよう、
// サーバーの現在値を端末の高水位マークと合成する。
//
// 何から守っとるか(2026-08-04 の調査で確定):
// サーバー側に集計値が減る経路は無い。measured_daily に delete は一箇所も無く、
// measured_saved は where 句を持たん単純合計で、日次の再同期も actual_minutes が
// 減る方向にしか動かん。つまりここが本当に守っとるのは「取得できんかった回」やった
// ── 通信断・認証切れ・RLS拒否のとき、以前は data ?? [] が 0 のスナップショットを
// 作り、圏外で開くたび庭が宣言前の空状態へ落ちとった。
//
// その入口は components/garden/load.ts が error を見るようになって塞がっとる。
// ここは二重の保険として残す ── 高水位そのものはサーバーの現在値と一致する値しか
// 持たんけん、サーバーへ昇格させる必要は無い(端末から失われても再取得で埋まる)。
