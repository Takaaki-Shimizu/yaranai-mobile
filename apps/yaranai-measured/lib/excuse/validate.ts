...
// 書ける長さの上限は理想(lib/ideal/validate.ts)と同じ全角20字。カードの版下は
// 全角14字を基準の最長ケースとして組んであり(§9-2)、それを超える行は描画側
// (bake.ts / preview-svg.ts)が安全幅(safeWidth)へ字送りごと縮めて収める。
//
...
/** 人が書く「やらないこと」の上限(全角換算)。理想(IDEAL_MAX_LENGTH)と同じ20字 */
export const EXCUSE_MAX_WIDTH = 20;
/** 1行のまま据えられる幅(全角換算)。これを超える宣言は2行に割り、長い行は描画側が縮める */
