...
| 中 | 庭の高水位マーク | 端末のみ。**この記述は `restore-flow-highwater-investigation.md` §6-1 で訂正済み** ── 高水位が守っているのは欠落日ではなく、Supabase の取得失敗時に `data ?? []` が 0 を作る経路(`components/garden/load.ts:59, 63`)。失われた端末は、それまで高水位が上乗せしていた分を失ってサーバーの実値から再開する | 復元完了時に `loadGrowth` の結果で初期化(コード追加のみ、マイグレーション不要) |
...
4. **6日を超える起動間隔で日次記録が恒久欠落する(Q4-2)。**
   ...
   新端末ではサーバーの実値から再開する。
   ※ここを当初「庭が後退する」と書いていたが、正しくは**後退ではなく高水位の上乗せ分の消失**である
   (`measured_daily` に削除経路は無く、集計値そのものは減らない)。
   詳細と訂正の根拠は `restore-flow-highwater-investigation.md` §6 を参照。
